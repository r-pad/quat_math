...
def quatMult(init_quat, max_orientation_offset):
    offset_axis = np.zeros(3)
    offset_axis[0] = 1
    offset_axis /= np.linalg.norm(offset_axis)
    norm_const = 1./2. * (max_orientation_offset - np.cos(max_orientation_offset)
                          * np.sin(max_orientation_offset))/_cdf_norm
    offset_angle = _inv_cdf(norm_const * 0.5)
    offset_quat = tf_trans.quaternion_about_axis(offset_angle, offset_axis)
    return tf_trans.quaternion_multiply(init_quat, offset_quat)
...
def quaternionBatchMultiply(q2, q1):
    return np.stack([q2[:, 0]*q1[:, 3] + q2[:, 1]*q1[:, 2] - q2[:, 2]*q1[:, 1] + q2[:, 3]*q1[:, 0],
                     ...
                     q2[:, 0]*q1[:, 1] - q2[:, 1]*q1[:, 0] +
                     q2[:, 2]*q1[:, 3] + q2[:, 3]*q1[:, 2],
                     -q2[:, 0]*q1[:, 0] - q2[:, 1]*q1[:, 1] - q2[:, 2]*q1[:, 2] + q2[:, 3]*q1[:, 3]], axis=1)
